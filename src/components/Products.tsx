      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 [perspective:1500px]">
        {products.map((p, i) => (
          <FadeInScope
            key={i}
            delay={i * 0.15}
            className="bg-card rounded-[28px] overflow-hidden border border-border hover:-translate-y-2.5 hover:shadow-2xl transition-all duration-300 group p-8 flex flex-col justify-between min-h-[320px]"
            initial={{ rotateX: 25, z: -100, opacity: 0 }}
            whileInView={{ rotateX: 0, z: 0, opacity: 1 }}
          >
            <div>
              <div className="flex justify-between items-start mb-6">
                <h3 className="text-[26px] font-extrabold tracking-[-0.5px] text-foreground lowercase">
                  {p.name}
                </h3>
                <span className="bg-foreground/10 backdrop-blur-lg border border-foreground/20 text-foreground text-[12px] font-bold px-3 py-1 rounded-full whitespace-nowrap">
                  {p.volume}
                </span>
              </div>
            ...
            <div>
              <span className="inline-block bg-primary/10 text-primary text-[11px] font-bold px-4 py-1.5 rounded-full tracking-wider uppercase">
                {p.tag}
              </span>
            </div>
          </FadeInScope>
